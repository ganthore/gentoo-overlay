...
IUSE="broadcom-sta ap dbus gnutls eap-sim fasteap +hs2-0 libressl p2p ps3 qt5 readline selinux smartcard ssl tdls uncommon-eap-types wimax wps kernel_linux kernel_FreeBSD"
...
src_prepare() {
	...
	if use broadcom-sta; then
		eapply "${FILESDIR}/${P}-base-fixes.patch"
		eapply "${FILESDIR}/${P}-driver-ext-c.patch"
		eapply "${FILESDIR}/${P}-driver-ext-h.patch"
	fi
